...
import (
	...
	"golang.zx2c4.com/go118/netip" // this dep is probably not needed, can do ourselves
...
func getOptionsFromFile(f string) (*options, error) {
	...
	for i, l := range lines {
		...
		p := strings.Split(l, "=")
		if len(p) != 2 {
			return nil, fmt.Errorf("wrong line (%s): more than one =", i)
		}
		k, v := p[0], p[1]
		switch k {
		case "ip":
...
func (m *Measurer) bootstrap(ctx context.Context, sess model.ExperimentSession,
	...
	sess.Logger().Infof(
		"wireguard: hello world this is logging: %+v", true)
	defer func() {
		out <- tk
	}()
